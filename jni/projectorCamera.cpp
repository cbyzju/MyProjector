...
void ProjectorCamera::processing(cv::Mat& iRSrc, cv::Mat& depthSrc)
{
   ...
   // screenRoi.height = 335;
	...
	{
		backGroundRefreshThread.join();
		throw;
	}

	//multi sub thread join to the main thread
	backGroundRefreshThread.join();
...
void ProjectorCamera::findFinger()
{
    ...
    //LOGD("nativeStart caught in findFinger result touchPoint: frameId: %d hands: %d", frameId,fingerTouchRole.curtTouchHands.size());
...
#pragma region keep stable
		...
		Eigen::Matrix<double, 3, 4> show;
		show << output_2[0].x, output_2[1].x, output_2[2].x, output_2[3].x,
			    output_2[0].y, output_2[1].y, output_2[2].y, output_2[3].y,
			    output_2[0].z, output_2[1].z, output_2[2].z, output_2[3].z;

		for (size_t ii = 0; ii < 4; ++ii)
		{
			show(0, ii) = -show(0, ii);
			show.col(ii) = ColToProR.inverse()*(show.col(ii) - ColToProT);
...
void ProjectorCamera::refineVerticals(vector<cv::Point3f>& verticals)
{
	...
	//model diagonal length
    double realDiagonal = SALVER_WIDTH_MM * sqrt(2.0);

    for(int ind=0;ind<2;ind++)
    {
		//detected object diagonal length
		...
		//refine detected object length according to bias
...
void ProjectorCamera::convertFloatToChar(cv::Mat& src, cv::Mat& dst)
{
	...
	//if src and dst mat data is not continue in memory
	else
	{
